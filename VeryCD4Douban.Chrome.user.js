    ...
    function _getHtml() {
        ...
        if (l > 0) {
            ...
            for (var i = 0; i < 3 && i < l; i++) {
            	alert(_itemTpl.process(_records[i]));
                s.push(_itemTpl.process(_records[i]));
            }
            s.push('<span id="_verycd4douban_more" style="display:none">');
            while (i < l) {
                s.push(_itemTpl.process(_records[i]));
                i++;
            }
            s.push('</span>');
            s.push('<a id="_verycd4douban_btn" href="javascript:void(0)">显示更多...</a>');
            s.push('</ul>');
        }
        s.push('</div></br>');
        return s.join('');
